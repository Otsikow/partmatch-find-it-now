...
import { Search, MapPin, Calendar, MessageCircle, Package, ClipboardList } from "lucide-react";
...
const SearchParts = () => {
  ...
  // For car parts
  const { parts, loading: partsLoading, error: partsError } = useCarParts({ searchTerm, filters });
  ...
  useEffect(() => {
    const filtered = requests.filter(request =>
      request.part_needed.toLowerCase().includes(requestSearchTerm.toLowerCase()) ||
      request.car_make.toLowerCase().includes(requestSearchTerm.toLowerCase()) ||
      request.car_model.toLowerCase().includes(requestSearchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-accent/5 to-background">
      <PageHeader 
        title={t('browseParts')}
        subtitle={t('findPartsAndRequests')}
        showBackButton={true}
        backTo="/"
      />
